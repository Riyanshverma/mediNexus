            ...
            {tabs.map(({ key, label, icon: Icon, badge }) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-4 px-4 py-3 text-sm transition-all text-left ${
                    isActive 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {(passport?.reports ?? []).length === 0 ? (
                    ...
                    (passport?.reports ?? []).map((report) => {
                      ...
                      return (
                        <div key={report.id} className="bg-card rounded-[24px] border border-border p-6 flex flex-col justify-between hover:border-primary/30 transition-colors group">
                          
                          {/* Header / Badges */}
                          <div className="flex items-start justify-between mb-6">
                            <div className="h-12 w-12 rounded-full bg-gradient-to-br from-primary/10 to-card border border-border flex items-center justify-center shrink-0">
                            ...
                            <span className="bg-muted text-primary text-[10px] uppercase font-bold tracking-widest px-3 py-1 rounded-full border border-border">
                               {report.report_category ?? report.report_type ?? 'Report'}
                            </span>
                          </div>

                          {/* Info Blocks */}
                          <div className="mb-6">
                            <h3 className="text-lg font-bold text-foreground leading-tight mb-1">{report.report_name}</h3>
                            <p className="text-xs text-muted-foreground">{(report as any).hospitals?.name ?? 'External Lab'}</p>
                            
                            <div className="flex items-center gap-4 mt-3 text-[11px] text-muted-foreground font-medium">
                              <span className="flex items-center gap-1.5"><Calendar className="h-3 w-3" /> {format(parseISO(report.uploaded_at), 'MMM d, yyyy')}</span>
                              <span className="flex items-center gap-1.5"><FileText className="h-3 w-3" /> PDF</span>
                            </div>
                          </div>

                          {/* Controls */}
                          <div className="pt-5 border-t border-border mt-auto space-y-3">
                             {/* Language toggle row */}
                             <div className="flex items-center justify-center rounded-lg bg-muted/50 border border-border p-1 w-full text-xs">
                               {(['en', 'hi'] as const).map((l) => (
                                 <button
                                   ...
                                   disabled={isLoading}
                                   className={`flex-1 py-1.5 font-bold rounded-md transition-colors ${
                                     lang === l
                             ...
                             <div className="flex gap-3">
                               <button
                                 ...
                                 className={`flex-1 rounded-full py-2.5 text-xs font-bold transition-all flex items-center justify-center gap-2 ${
                                   isError
                                     ? 'border border-red-500/20 text-red-500 bg-red-500/5'
                                     : isPlaying
                                 ...
                                 {isLoading ? (
                                   ...
                                   <>
                                     <span className="flex items-end gap-[2px] h-3.5">
                                       {[0, 1, 2, 3].map((i) => (
                                         <span key={i} className="w-[3px] rounded-full bg-primary" style={{ height: '100%', animation: `speakBar 0.8s ease-in-out infinite`, animationDelay: `${i * 0.15}s`, transformOrigin: 'bottom' }} />
                                       ))}
                                     </span>
                                     Pause
                                   </>
                                   ...
                                   <>
                                     <span className="flex items-end gap-[2px] h-3.5 opacity-70">
                                       {[2, 4, 3, 5].map((h, i) => (
                                         <span key={i} className="w-[3px] rounded-full bg-current" style={{ height: `${h * 14}%` }} />
                                       ))}
                                     </span>
                                     Listen
                                   </>
                               ...
                               <a
                                 ...
                                 className="flex-1 bg-muted hover:bg-muted/80 text-foreground rounded-full flex justify-center items-center gap-2 text-xs font-bold transition-all"
                               >
